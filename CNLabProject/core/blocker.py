...
def managed_content(content, sites):
    starts = [i for i, line in enumerate(content) if line == MARKER_START]
    ends = [i for i, line in enumerate(content) if line == MARKER_END]
    if len(starts) > 1 or len(ends) > 1:
        raise ValueError('Duplicate parental-control markers in hosts file')
    if bool(starts) != bool(ends) or (starts and starts[0] >= ends[0]):
        raise ValueError('Malformed parental-control markers in hosts file')
    block = [MARKER_START]
    for site in sites:
        block.extend((f'127.0.0.1 {site}\n', f'::1 {site}\n'))
    block.append(MARKER_END)
    if starts:
        return content[:starts[0]] + block + content[ends[0] + 1:]
    separator = ['\n'] if content and not content[-1].endswith('\n') else []
    return content + separator + block


def atomic_write(lines):
    backup = HOSTS_PATH.with_suffix('.parental-control.bak')
    ...
    with hosts_lock:
        try:
            ...
            if len(starts) == len(ends) == 1 and starts[0] < ends[0]:
                atomic_write(content[:starts[0]] + content[ends[0] + 1:])
                flush_dns()
        except PermissionError:
