...
def dashboard_status():
    try:
        with urllib.request.urlopen(DASHBOARD_URL + '/health', timeout=1) as response:
            if response.status != 200:
                return None
            return bool(json.load(response).get('protection'))
...
class ChildSafeDesktop:
    ...
    def refresh(self):
        state = service_state()
        protection = dashboard_status() if state == 'RUNNING' else None
        if state == 'RUNNING' and protection is True:
        ...
        elif state == 'RUNNING' and protection is False:
